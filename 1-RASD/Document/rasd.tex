...
\imagebox{../Mockups/Web.png}{Web-application view on personal computers}{0}
\imagebox{../Mockups/Mobile.png}{Web-application view on mobile devices}{0}
...
\imagebox{../UML_Model/UML_img/General_UseCase__General_UseCaseDiagram_4}{Use Case Diagram}{0}
\subsubsection{Descriptions}
\usecasedesc{\textbf{Registration}}
{User not registered, the system}
{There are no entry conditions}
{\begin{enumerate}
		\item The user compiles the form with his personal data.
		\item The user inserts his external payment account.
		\item The user inserts his driving license number.
		\item The user confirms the registration thanks to the mail sent by the system.
\end{enumerate}}
{The user has an account and the system recognizes him}{\begin{itemize}
\item The external payment account doesn't exist.
\item The driving license number doesn't exist or is invalid.
\item Some of the personal data are not accepted (for example a small password).
\end{itemize}In any of these cases the system asks the user to reinsert correct data.}

\usecasedesc{\textbf{Search for a car}}{ User registered, the system}{The user must be logged into the system}{\begin{enumerate}
		\item The user goes to the search interface.
		\item The user decides to search for a car manually navigating the map or specifying a place or automatically using the GPS location system.
		\item The user selects the range within he wants to find the car.
		\item The system shows the available cars and the near parking areas.
\end{enumerate}}{The user sees on the display a list of cars or a map with cars and parking area positions}{\begin{itemize}
\item The location doesn't exist.
\item GPS is off.
\end{itemize}All of these cases are solved displaying an error to the user. 
For example: if there are no cars "NO CARS AVAILABLE", if the GPS is off "GPS IS OFF".}

\usecasedesc{\textbf{Reserve a car}}{User registered, the system}{User must be logged in and must have a selected car  thanks to the previous search}{\begin{enumerate}
	\item User confirms the reservation with a duration of 1 hour.
\end{enumerate}}{The user has reserved a car that he is be able to take in an hour. He can check the reservation in the "MANAGE RESERVATION" panel and possibly cancel it. This car is marked as reserved.}{\begin{itemize}
\item User's balance isn't positive.
\end{itemize}
The system displays an error and brings him back to the initial page.}

\usecasedesc{\textbf{Pick up a car}}{User registered, the system}{User must be logged in}{\begin{enumerate}
		\item User wants to unlock a car which is not reserved by someone else
		\item The system unlocks the car verifying the user GPS position and the user balance
		\item User enters into the car
\end{enumerate}}{The user can enter and drive the car, starting by clicking "START" on the car display}{\begin{enumerate}
\item The system can't verify the user position because the user GPS is off.
The system asks the user to turn the GPS on.
\item User has a non-positive balance.
\end{enumerate}The system displays an error to and brings him back to the initial page.}

\usecasedesc{\textbf{Park the car}}{User, the system}{User must be logged in and must have taken a car}{\begin{enumerate}
		\item User parks the car in a \textit{Safe Area} or in \textit{Charging Area}
		\item User powers off the car.
		\item User notifies on the car-display that the ride is over.
		\item The system displays the cost of the service showing possible discounts or extra charges.
		\item The system locks the car.
\end{enumerate}}{User exits the car and pays the service(with an external system)}{\begin{enumerate}
	\item The user can't afford the ride.
	\item The user doesn't click on the finish button on the car display.
\end{enumerate}In both of these cases the external system payment is responsible.}

\subsection{Class Diagram}
\imagebox{../UML_Model/UML_img/General_Class__General_ClassDiagram_3}{Class Diagram}{0}
\subsection{Sequence Diagram}
\imagebox{../UML_Model/UML_img/Charge_Sequence__Interaction1__Charge_SequenceDiagram_9}{Charge Sequence Diagram}{0}
\imagebox{../UML_Model/UML_img/p1}{Reservation Sequence Diagram Part 1}{0}
\imagebox{../UML_Model/UML_img/p2}{Reservation Sequence Diagram Part 2}{0}
\imagebox{../UML_Model/UML_img/p3}{Reservation Sequence Diagram Part 3}{0}
\subsection{Statechart Diagram}
\imagebox{../UML_Model/UML_img/Car_StateChart__Car_StateChartDiagram_0}{Car status Statechart}{0}
\subsection{Activity Diagram}
\imagebox{../UML_Model/UML_img/Charge_Activity__Charge_ActivityDiagram_1}{Charge Activity Diagram}{0}
\imagebox{../UML_Model/UML_img/Check_Activity__Check_ActivityDiagram_2}{Check Activity Diagram}{0}
\imagebox{../UML_Model/UML_img/Registration_Activity__Registration_ActivityDiagram_6}{Registration Activity Diagram}{0}
\imagebox{../UML_Model/UML_img/Reservation_Activity__Reservation_ActivityDiagram_7}{Reservation Activity Diagram}{0}
\section{Alloy}
