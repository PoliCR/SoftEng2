...
\end{Goal}
Furthermore, because the system will be used also from company employees, it has to achieve the next goals with respect to the \textbf{\textit{Employee}}. 
\begin{Goal}[resume]
\item \itemtext{G9} The employee can log in into the system
\item \itemtext{G10} The employee can search for the cars over the city
\item \itemtext{G11} The employee can update the status of the cars
\end{Goal}
...
\indent \\
\textbf{Registered User}: interacts with the system to use the sharing service. He has an account (which contains personal information, driving license number and payment data) that must be used to accede to the application to exploit all the functionality. \\
\noindent \\
...
In this first chapter, after a brief description of the starting problem, the explanation of the system-to-be and its application domain (according to the \textit{World and the Machine Paper}), the goals that the system has to achieve to fulfill the users and employees needs have been stated out.
Finally, there is a glossary that help to identify and define in a unique way some words and concepts, that will be used later on in this document and in the next ones, that could be misunderstood.\\
In the second chapter, there is small presentation of product perspective and main functions; after that, it is possible to find a basic analysis and characterization of the beneficiaries (users and employees) of the system-to-be. The last part of this chapter covers the assumptions that have been made in order to develop the system.\\
In the third chapter, which is the core of the RASD, the Functional Requirements are derived from the goals defined in the first chapter. Next, will be presented Non Functional Requirements.\\
The fourth chapter gathers a bunch of scenarios for all the possible usage of the system.\\
In the fifth chapter, the Functional Requirements of the system-to-be are modeled firstly using a semi-formal notation such a UML, via different types of diagrams, and secondly through Alloy code; then, with an Alloy tool, the written model has been properly verified and also some generated worlds analyzed.
In the sixth chapter are listed the tools that have been used to redacting the document and reported the time effort required to complete the document.
...
\end{Req}
[DA6]
%TODO deriving req for employees
...
- The user bind a driving license.
...
- The external driving license doesn't exist or is invalid.
...
- The user decides to search for a car manually scrolling the map, specifying a place or automatically thanks to GPS location.
...
Exit conditions: The user has reserved a car that he will takes in an hour. He can check the reservation in the "MANAGE RESERVATION" screen, where he can also delete it. Other users sees this car as reserved.
Exceptions:
...
- The system unlocks the car verifying the user GPS position.
