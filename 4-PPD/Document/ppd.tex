...
\end{tabularx}
\\\\
\textit{User Data}, \textit{Reservation Data} and \textit{Ride Data} have a \textit{mid} because they have more attributes that will be manage. Differently, \textit{Employee Data}, \textit{Safe Area Data} and \textit{Car Data} have a \textit{Low} complexity because they have few attributes.
...
\textit{Car Plug In} has an \textit{High} complexity because the car has to interact to the System.\\
\textit{Login}, \textit{Logout}, \textit{Registration}, \textit{Update Car Status}, \textit{Convey end ride} and \textit{Insert Final Destination} are all considered as \textit{Mid} or \textit{Low} complexity because they manage few data or the communication is created between Mobile Phone - System.
...
\begin{equation}
\centerline{\textit{Code Lines =} $184\cdot$($34\textsubscript{HTML}\cdot$0.15\textsubscript{HTML} + $46\textsubscript{JEE}\cdot$0.75\textsubscript{JEE} + $43\textsubscript{JS}\cdot$0.1\textsubscript{JS}) \textit{= \textbf{8078}}}
\end{equation}
\newpage
